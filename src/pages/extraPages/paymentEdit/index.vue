<template>
 ...
 <!-- 供应商订单信息 ---产品列表-->
 <Purchase :paymentList='paymentList' v-if="showDetail" :addPage="true"></Purchase>
      ...
      data['payment'].getData(params).then((res) => {
        ...
        if(data.paymentType == 1){
         ...
         this.photoList = data.fileList
          ...
          : [];
           this.uuid=data.batchId?data.batchId:this.data.get_uuid()
      });
      let params2 = {
        formId: 11,
      };
    ...
    save(val) {
      ...
      if (this.needList.length > 0) {
        data.upLoadFile(this.needList, 0, this.uuid).then((res) => {
          ...
          data["payment"].editOrStart(params).then((res) => {
